...
% ---------------------------------------------------------------
\begin{abstract}
Large clinical datasets are hard to build for Internet of Medical Things (IoMT) applications. Privacy rules and the rarity of many diseases keep cohorts small. The Mayo Clinic Primary Biliary Cirrhosis (PBC) data show this clearly: 418 enrolled patients drop to 276 usable records once incomplete cases are removed. We ask a simple question. Does adding synthetic patient records to the training set actually improve prediction on real patients? We build three generative models from scratch in TensorFlow 2.15 under fixed random seeds: a Vanilla GAN, a conditional tabular GAN (CTGAN), and a Tabular Variational Autoencoder (TVAE). Generated records are filtered for clinical plausibility with IQR-based Tukey fences, and a consensus step then keeps only the records that all three models agree on. We measure distributional quality with a tabular Fr\'{e}chet Inception Distance (FID), Kolmogorov-Smirnov tests, Jensen-Shannon divergence, Cohen's $d$, and chi-square tests, and we measure privacy risk from nearest-neighbour distances. Five scenarios are tested by training Random Forest, Gradient Boosting, and Logistic Regression and scoring each on 83 held-out real patients, using 5-fold cross-validation that keeps synthetic records on the training side of every fold, 1000-resample bootstrap intervals, and McNemar's exact test. CTGAN gives the best distributional quality at FID~0.0724, and the filtered adversarial pools stay at low disclosure risk (near-duplicate rates of 7.3\% for GAN and 9.6\% for CTGAN). Once the evaluation avoids leakage, no scenario beats the real-data baseline. Every McNemar test gives $p > 0.05$, all bootstrap intervals overlap, and cross-validation AUC neither rises nor grows more stable (the Gradient Boosting AUC standard deviation moves only from 0.071 to 0.054). Consensus voting does not lower disclosure risk either, at a 21.1\% near-duplicate rate, because it keeps records that sit close to real patients. A classifier trained on synthetic CTGAN data alone still reaches AUC 0.82 to 0.84 for Random Forest and Logistic Regression on real patients, so the synthetic data does carry usable predictive signal even where augmentation adds none. The lesson is plain: looking similar is not the same as being useful for this small tabular cohort, and careful held-out, leakage-free testing must come before synthetic augmentation is trusted in IoMT decision support.
\end{abstract}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig02_pipeline_flowchart.png}
...
\end{equation}
with $\beta = 1.0$, Adam optimiser at learning rate $1 \times 10^{-3}$, batch size 32, and 300 training epochs. Fig.~\ref{fig:training} shows the training loss curves for all three models over their respective epochs.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig03_training_losses.png}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig04_iqr_filtering.png}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig05_consensus_distribution.png}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig06_fid_comparison.png}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig07_distribution_comparison.png}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig08_correlation_heatmap.png}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig09_tsne_projection.png}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig10_privacy_risk_analysis.png}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig13_auc_comparison.png}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig14_performance_heatmap.png}
  \caption{F1-score heatmap across all four scenarios (A through D) for all three classifiers. Darker blue indicates higher F1. Gradient Boosting under Scenario D achieves the highest F1 at 0.809. Under Scenario C, Gradient Boosting degrades to 0.726 while Logistic Regression rises to 0.785 and Random Forest falls to 0.738.}
  \label{fig:heatmap}
\end{figure}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig15_model_comparison.png}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig16_all_metrics.png}
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\columnwidth]{fig17_feature_importance.png}
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\columnwidth]{fig18_power_analysis.png}
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\columnwidth]{fig19_subgroup_analysis.png}
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\columnwidth]{fig20_privacy_enhancement.png}
